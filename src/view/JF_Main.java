...
public class JF_Main extends JFrame implements MouseListener, ActionListener, MouseMotionListener {
	...
	private Pnl_Add_Word pnl_add_word;
	...
	// variables estaticas.
	// static public List<Word> lista_words = new ArrayList<Word>(); // lista grupo
	// 0, su valor se obtiene en el metodo readFile(), del servicio.
	...
	private int grupoHojaActual = 0; // para saber que grupo es el que se esta mostrando en la pantalla principal.
	private int panelActivo = 0; // 1 -> hoja(g1,g2,...), 2 -> add_word
//	boolean [] botones = {true, false, false}; // para saber que botones estan activos.
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					UIManager.setLookAndFeel("com.sun.java.swing.plaf.windows.WindowsLookAndFeel"); // --> CAMBIA EL
																									// LOOK AND FEEL (
																									// DISENIO )
					SwingUtilities.updateComponentTreeUI(frame); // <<<
					// frame.setShape(new RoundRectangle2D.Double(xRound, yRound, 400, 600, 50,
					// 50));
...
//		pnl_superior.setBorder(new LineBorder(Color.cyan));
		...
		btn_add.setBounds(640, 1, 28, 28);
		pnl_strip.add(btn_add);
		...
		setCantidadPalabras(lista_words_grupo_01);
		setCantidadPaginas(lista_words_grupo_01);
		setTotalPalabras();
	...
	void setCantidadPalabras(List<Word> list) {
		cantidad_palabras = list.size();
	}

	void setCantidadPaginas(List<Word> list) { // NO PUEDE SER 0
...
//		String total = Integer.toString(cantidad_palabras);
	...
	int getCantidadPalabrasTotales() {
		int total = 0;
		total = lista_words_grupo_01.size() + lista_words_grupo_02.size() + lista_words_grupo_03.size()
				+ lista_words_grupo_04.size() + lista_words_grupo_05.size();
		return total;
	}

	void cambiarPanelHoja() { // cambiar al panel hoja
		if (panelActivo == 1)
			return;
	...
	void cambiarPanelAddWord() { // cambiar al panel add Word
		if (panelActivo == 2)
			return;
		pnl_add_word = new Pnl_Add_Word();
		MostrarEnPanelPrincipal(pnl_add_word);
		panelActivo = 2;
		grupoHojaActual = 0;
	}
...
//		pnl_principal.revalidate();
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btn_grupo01 && grupoHojaActual != 1) {
			cambiarPanelHoja(); // si ya esta activo el panel hoja no lo vuelve a cargar.
			setCantidadPalabras(lista_words_grupo_01);
			setCantidadPaginas(lista_words_grupo_01);
			pagina_actual = 1;
		...
		if (e.getSource() == btn_grupo02 && grupoHojaActual != 2) {
			cambiarPanelHoja(); // si ya esta activo el panel hoja no lo vuelve a cargar.
			setCantidadPalabras(lista_words_grupo_02);
			setCantidadPaginas(lista_words_grupo_02);
			pagina_actual = 1;
			Pnl_Hoja.lbl_pagination.setText(JF_Main.pagina_actual + "/" + JF_Main.cantidad_paginas);
		...
		if (e.getSource() == btn_grupo03 && grupoHojaActual != 3) {
			cambiarPanelHoja(); // si ya esta activo el panel hoja no lo vuelve a cargar.
			setCantidadPalabras(lista_words_grupo_03);
			setCantidadPaginas(lista_words_grupo_03);
			pagina_actual = 1;
			Pnl_Hoja.lbl_pagination.setText(JF_Main.pagina_actual + "/" + JF_Main.cantidad_paginas);
		...
		if (e.getSource() == btn_grupo04 && grupoHojaActual != 4) {
			cambiarPanelHoja(); // si ya esta activo el panel hoja no lo vuelve a cargar.
			setCantidadPalabras(lista_words_grupo_04);
			setCantidadPaginas(lista_words_grupo_04);
			pagina_actual = 1;
			Pnl_Hoja.lbl_pagination.setText(JF_Main.pagina_actual + "/" + JF_Main.cantidad_paginas);
		...
		if (e.getSource() == btn_grupo05 && grupoHojaActual != 5) {
			cambiarPanelHoja(); // si ya esta activo el panel hoja no lo vuelve a cargar.
			setCantidadPalabras(lista_words_grupo_05);
			setCantidadPaginas(lista_words_grupo_05);
			pagina_actual = 1;
			Pnl_Hoja.lbl_pagination.setText(JF_Main.pagina_actual + "/" + JF_Main.cantidad_paginas);
